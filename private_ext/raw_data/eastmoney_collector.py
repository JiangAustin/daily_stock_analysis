...
class EastMoneyRawDataCollector(RawDataCollector):
    ...
    def collect(self, symbol: str, trade_date: str) -> RawStockData:
        ...
        quality_report = build_quality_report(
            ...
            field_values={
                "market_snapshot.close": first_non_missing(market_snapshot.get("close")),
                "market_snapshot.pct_change": first_non_missing(market_snapshot.get("pct_change")),
                "valuation_raw.pe": first_non_missing(valuation_raw.get("pe")),
                "valuation_raw.pb": first_non_missing(valuation_raw.get("pb")),
                "financial_raw.roe": financial_raw.get("roe"),
                "financial_raw.net_profit_growth": financial_raw.get("net_profit_growth"),
                "kline_summary.return_20d": first_non_missing(kline_summary.get("return_20d")),
    ...
    def _run_groups(
        ...
        *,
        ...
        field_values = {
            "market_snapshot.close": first_non_missing(
                group_fields.get("snapshot", {}).get("close"),
                group_fields.get("kline", {}).get("close"),
            ),
            "market_snapshot.pct_change": first_non_missing(
                group_fields.get("snapshot", {}).get("pct_change"),
                group_fields.get("kline", {}).get("pct_change"),
            ),
            "valuation_raw.pe": first_non_missing(
                group_fields.get("snapshot", {}).get("pe"),
                group_fields.get("valuation", {}).get("pe"),
            ),
            "valuation_raw.pb": first_non_missing(
                group_fields.get("snapshot", {}).get("pb"),
                group_fields.get("valuation", {}).get("pb"),
            ),
            "financial_raw.roe": first_non_missing(group_fields.get("financial", {}).get("roe")),
            "financial_raw.net_profit_growth": first_non_missing(group_fields.get("financial", {}).get("net_profit_growth")),
            "kline_summary.return_20d": first_non_missing(group_fields.get("kline", {}).get("return_20d")),
            "kline_summary.return_5d": first_non_missing(group_fields.get("kline", {}).get("return_5d")),
            "kline_summary.return_60d": first_non_missing(group_fields.get("kline", {}).get("return_60d")),
        }
        diagnostics = EastMoneyDiagnosticsReport(
            ...
            unresolved_fields=[field for field, value in field_values.items() if _is_missing(value)],
